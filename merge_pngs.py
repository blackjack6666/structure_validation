...
images = glob('F:/native_digestion/01242023/3d_mapping/*.png')
...
images_slice = [each for each in chunks(images, 13)]
# images_slice = list(zip(trypsin_images, chymotrypsin_image))
print(len(images_slice))
counter = 1
for each in images_slice:
    ...
    new_im.save(
        'F:/native_digestion/01242023/3d_mapping_combined/' + each[0].split('\\')[-1].split('_')[
            0] + 'time_combined.png')
    counter += 1
